...
function formatDate(timestamp?: { toDate: () => Date }) {
  return timestamp?.toDate().toLocaleString('zh-TW', { dateStyle: 'medium', timeStyle: 'short' }) ?? '—'
}
...
function HostRecordsPanel({ hostUid, activeRoundId }: { hostUid: string; activeRoundId?: string }) {
  ...
  return <section className="host-records" aria-labelledby="class-records-heading">
    <div className="records-heading"><div><span className="eyebrow">講師專用紀錄</span><h2 id="class-records-heading">課堂紀錄</h2><p>查看此講師帳號擁有的所有計分輪次、學員、題目答案與作答紀錄。</p></div><button className="secondary-button" disabled={records.loading} onClick={records.refresh}>{records.loading ? '載入中…' : '重新整理紀錄'}</button></div>
    {records.error && <p className="form-error panel inline-error" role="alert">{records.error}</p>}
    <div className="records-summary"><div className="panel"><span>計分輪次</span><strong>{records.rounds.length}</strong></div><div className="panel"><span>學員紀錄</span><strong>{playerCount}</strong></div><div className="panel"><span>題目代碼</span><strong>{records.questions.length}</strong></div><div className="panel"><span>已送出答案</span><strong>{answerCount}</strong></div></div>
    {records.loading ? <LoadingPanel text="載入課堂紀錄中…" /> : <div className="records-columns">
      <section><h3>計分輪次與學員</h3>{records.rounds.length ? records.rounds.map(round => <details className="panel record-card" key={round.id}><summary><div><strong>{round.id === activeRoundId ? '目前輪次' : '歷史輪次'}</strong><small>{formatDate(round.createdAt)}</small></div><span>{round.players.length} 位學員</span></summary><div className="records-table-wrap"><table><thead><tr><th>學員</th><th>學員 ID</th><th>分數</th><th>答對累計時間</th><th>加入時間</th></tr></thead><tbody>{round.players.length ? [...round.players].sort(comparePlayers).map(player => <tr key={player.id}><td><strong>{player.nickname}</strong></td><td><code>{player.id}</code></td><td>{player.score.toLocaleString()}</td><td>{player.tieBreakTimeMs === undefined ? '—' : `${(player.tieBreakTimeMs / 1000).toFixed(1)} 秒`}</td><td>{formatDate(player.joinedAt)}</td></tr>) : <tr><td colSpan={5}>本輪尚無學員。</td></tr>}</tbody></table></div></details>) : <div className="panel records-empty">尚未建立計分輪次。</div>}</section>
      <section><h3>題目、正確答案與作答紀錄</h3>{records.questions.length ? records.questions.map(question => <details className="panel record-card" key={question.code}><summary><div><strong>{question.code}</strong><small>{question.isOpen ? '開放作答中' : '已關閉'} · 輪次 {question.roundId}</small></div><span>{question.answers.length} 份答案</span></summary><div className="question-record-meta"><span>正確答案</span><strong>{question.answerKey.length ? question.answerKey.map(value => optionLabels[value]).join('、') : '尚未設定'}</strong><span>Session {question.sessionNumber} · Question {String(question.questionNumber).padStart(2, '0')}</span></div><div className="records-table-wrap"><table><thead><tr><th>學員</th><th>送出答案</th><th>狀態</th><th>得分</th><th>作答時間</th><th>送出時間</th></tr></thead><tbody>{question.answers.length ? [...question.answers].sort((first, second) => (first.submittedAt?.toMillis() ?? 0) - (second.submittedAt?.toMillis() ?? 0)).map(answer => <tr key={answer.id}><td><strong>{playerNames.get(`${answer.roundId}:${answer.studentUid}`) ?? '未知學員'}</strong><small><code>{answer.studentUid}</code></small></td><td>{answer.selections.map(value => optionLabels[value]).join('、')}</td><td>{answer.awarded ? '已計分' : '待結算'}</td><td>{answer.points === undefined ? '—' : answer.points.toLocaleString()}</td><td>{answer.responseTimeMs === undefined ? '—' : `${(answer.responseTimeMs / 1000).toFixed(1)} 秒`}</td><td>{formatDate(answer.submittedAt)}</td></tr>) : <tr><td colSpan={6}>本題尚無作答紀錄。</td></tr>}</tbody></table></div></details>) : <div className="panel records-empty">尚未建立題目代碼。</div>}</section>
    </div>}
  </section>
}
